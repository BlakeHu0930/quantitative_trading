...
pip install pandas numpy matplotlib longport python-dotenv tqdm
```
...
src/
  ...
  signals.py                ← generate_signals(df, params) — 含 Signal_Reason 列
  backtest.py               ← backtest(df, capital) → (df, metrics, trades)
  portfolio.py              ← analyze_portfolio(stock_results)
  optimizer.py              ← optimize_strategy(df, param_ranges, metric)
